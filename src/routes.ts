...
export const routes: Record<string, RouteConfig> = {
  ...
  "New Releases": { path: "/new-releases", label: "New Releases", parent: "Browse" },
  Trending: { path: "/trending", label: "Trending", parent: "Browse" },
  "Top Rated": { path: "/top-rated", label: "Top Rated", parent: "Browse" },
  Collections: { path: "/collections", label: "Collections", parent: "Browse" },
  ...
  Documentaries: { path: "/documentaries", label: "Documentaries", parent: "Categories" },
...
export function getPageName(path: string): string {
  const entry = Object.entries(routes).find(([, config]) => config.path === path);
